"""
api_server.py — FastAPI backend for the React UI.
Imports existing pipeline functions from scripts/ without modifying them.
...
# ─── Prompt for tab 1 (generic extraction) ───
...
# ─── FastAPI app ───
app = FastAPI(title="Extraction Factures BTP API")
...
# ─── Helpers ───
def extract_gemma2_texte(original_filename: str):
    """Tab 1: extraction with pre-computed OCR + generic prompt."""
...
def process_single_file(file_bytes, suffix, choix_fournisseur):
    """Tab 3: full pipeline — OCR DocTR in memory + Gemma2 Smart."""
    pil_images = []
    if suffix == ".pdf":
...
def build_excel_batch(results_list):
    """Generate styled Excel with one row per invoice."""
...
def build_excel_multi_sheets(results_list):
    """Generate multi-sheet Excel: one sheet per installateur + ALL."""
    from openpyxl.styles import Font, PatternFill, Alignment, Border, Side
...
@app.get("/api/installateurs")
def list_installateurs():
    noms = [k for k in SMART_INSTALLATEURS.keys() if k != "DEFAULT"]
    noms.sort()
    return {"installateurs": noms}


@app.post("/api/extract-texte")
async def api_extract_texte(file: UploadFile = File(...)):
    """Tab 1: Gemma2 Texte — uses pre-computed OCR."""
    fields, error = extract_gemma2_texte(file.filename)
    if error:
        return {"error": error, "fields": None}
    return {"fields": fields, "error": None}


@app.post("/api/extract-smart")
async def api_extract_smart(
    file: UploadFile = File(...),
    fournisseur: str = Form("Auto-detect"),
):
    """Tab 2: Gemma2 Smart — uses pre-computed OCR + specialized prompts."""
    texte_ocr = smart_get_ocr_text(file.filename)
    if texte_ocr is None:
        return {"error": "OCR introuvable pour cette facture", "fields": None, "installateur": None}

    if fournisseur == "Auto-detect":
        installateur = smart_detect_installateur(texte_ocr)
    elif fournisseur == "DEFAULT (generique)":
        installateur = "DEFAULT"
    else:
        installateur = fournisseur.lower()

    fields = smart_extraire_champs(texte_ocr, installateur)
    return {"fields": fields, "installateur": installateur, "error": None}
...
async def api_ocr_extract(
    ...
    fields, error, installateur = process_single_file(file_bytes, suffix, fournisseur)
    if error:
        return {"error": error, "fields": None, "installateur": None}
    return {"fields": fields, "installateur": installateur, "error": None}


@app.post("/api/batch-zip")
async def api_batch_zip(file: UploadFile = File(...)):
    """Tab 4: Batch processing — ZIP of invoices."""
    ...
    if not file_names:
        return {"error": "Aucun fichier PDF/PNG/JPG trouve dans le ZIP.", "results": []}
    ...
    for fname in file_names:
        ...
        # Fallback: DocTR in memory
        try:
            file_bytes = zf.read(fname)
            suffix = os.path.splitext(fname)[1].lower()
            fields, error, installateur = process_single_file(file_bytes, suffix, "Auto-detect")
    ...
    return {"results": batch_results, "error": None}


@app.post("/api/export-excel")
async def api_export_excel(body: dict):
    """Generate styled Excel from extraction results."""
    results = body.get("results", [])
    excel_bytes = build_excel_batch(results)
...
async def api_export_excel_multi(body: dict):
    """Generate multi-sheet Excel from batch results."""
    results = body.get("results", [])
    excel_bytes = build_excel_multi_sheets(results)
    return StreamingResponse(
        io.BytesIO(excel_bytes),
        media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        headers={"Content-Disposition": "attachment; filename=batch_multi.xlsx"},
    )
